	...
	<div class="middle" style="background:url(plugins_splash.jpg) top 300px no-repeat;height:600px;margin-left:20px;">
		...
		<div style="width:90%;height:400px;margin-top:95px;">
			...
			<div style="float:left;width:100%;height:80px;font-weight:normal;background:#F7F7F7;padding:5px 0 0 5px;">
				<span style="font-size:14px;font-weight:bold;"><?=$aPlugin->name?></span> (<?=$aPlugin->php?>,<?=$aPlugin->mysql?>)<br />
				by <?=$aPlugin->author?><br /><br  />

				<?=$aPlugin->description?><br />
			</div>
			<div style="float:left;text-align:right;width:100%;margin:0;padding:0;height:20px;margin-bottom:20px;"><a href="./download-plugin.php?id=<?=$aPlugin->pluginId?>">Download</a></div>
			<?php
				}
			?>
		</div>
