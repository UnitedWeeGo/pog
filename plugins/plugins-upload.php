			...
			<div style="float:left;width:100%;height:410px;font-weight:normal;background:#F7F7F7;padding:5px 0 0 5px;">
				...
				<table>
					<tr>
						...
						<td width="10"></td>
						<td><input name="name" class="i" style="width:250px;" value="<?=@$_SESSION['name']?>"></input></td>
						<td class="error"><?=(sizeof($_POST) > 0 && empty($_SESSION['name'])?"Missing name":"")?></td>
					...
					<tr>
						...
						<td width="10"></td>
						<td><input class="i" style="width:250px;" name="author" value="<?=@$_SESSION['author']?>"></input></td>
						<td class="error"><?=(sizeof($_POST) > 0 && empty($_SESSION['author'])?"Missing author":"")?></td>
					</tr>
					<tr>
						<td align="right">PHP Compatibility:</td>
						<td width="10"></td>
						<td><select class="s" style="width:250px;height:20px;" name="php">
							<option value="PHP 4" <?= @($_SESSION['php']=="PHP 4" ? "selected" : "") ?>>PHP 4</option>
							<option value="PHP 5" <?= @($_SESSION['php']=="PHP 5" ? "selected" : "") ?>>PHP 5</option>
							<option value="PHP 5.1" <?= @($_SESSION['php']=="PHP 5.1" ? "selected" : "") ?>>PHP 5.1</option>
						</select>
						</td>
						<td class="error"></td>
					</tr>
					<tr>
						...
						<td><select class="s" style="width:250px;height:20px;" name="mysql">
							<option value="MySQL 4" <?= @($_SESSION['mysql']=="MySQL 4" ? "selected" : "") ?>>MySQL 4</option>
							<option value="MySQL 5" <?= @($_SESSION['mysql']=="MySQL 5" ? "selected" : "") ?>>MySQL 5</option>
						</select>
						</td>
						<td class="error"></td>
					</tr>
					...
					<tr>
						...
						<td width="10"></td>
						<td><textarea class="t" name="description" style="width:250px;height:50px;" cols="10" rows="1"><?=@$_SESSION['description']?></textarea></td>
						<td class="error"><?=(sizeof($_POST) > 0 && empty($_SESSION['description'])?"Missing description":"")?></td>
					</tr>
					...
					<tr>
						...
						<td width="10"></td>
						<td><textarea class="t" name="code" style="width:250px;height:150px;" cols="10" rows="1"><?=@$_SESSION['code']?></textarea></td>
						<td class="error"><?=(sizeof($_POST) > 0 && empty($_SESSION['code'])?"Missing code":"")?></td>
					</tr>
					<tr>
						<td></td>
						<td width="10"></td>
