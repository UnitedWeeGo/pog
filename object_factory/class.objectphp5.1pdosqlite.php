...
class Object
{
	...
	function CreateSQLQuery()
	{
		...
		{
			case "sqlite":
				$this->sql .= "\n\tCREATE TABLE ".strtolower($this->objectName)." (\n\t".strtolower($this->objectName)."id INTEGER NOT NULL PRIMARY KEY,";
				$x=0;
				foreach ($this->attributeList as $attribute)
				{
					if ($this->typeList[$x] == "BELONGSTO")
					{
						...
						{
							$this->sql .= "\n\t".strtolower($attribute)."id INTEGER NOT NULL";
						}
						else
						{
							$this->sql .= "\n\t".strtolower($attribute)."id INTEGER NOT NULL, ";
	...
	function CreatePreface()
	{
		...
		$this->string .= "\n* @version POG ".$GLOBALS['configuration']['versionNumber'].$GLOBALS['configuration']['revisionNumber']." / PHP5.1 SQLITE";
