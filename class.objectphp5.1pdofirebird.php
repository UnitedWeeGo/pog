...
class Object
{
	...
	function CreateGetFunction()
	{
		...
		foreach ($this->attributeList as $attribute)
		{
			$this->string .="\n\t\t\t\t\$this->".strtolower($attribute)." = \$row['".strtoupper($attribute)."'];";
		}
		$this->string .="\n\t\t\t}";
		$this->string .="\n\t\t\treturn \$this;";
		$this->string .="\n\t\t}";
		...
		switch ($this->pdoDriver)
		{
			case "firebird":
				...
				foreach ($this->attributeList as $attribute)
				{
					...
					{
						$this->sql .= "\n\t".strtoupper($attribute)." ".$this->optionList[$x];
					}
					else
					{
						$this->sql .= "\n\t".strtoupper($attribute)." ".$this->optionList[$x].",";
	...
	function CreateSaveFunction()
	{
		...
		$this->string .= "\n\t\t\t\$count=0;";
		$this->string .= "\n\t\t\t\$sql = \"select count(".strtolower($this->objectName)."id) from ".strtolower($this->objectName)." where ".strtolower($this->objectName)."id = '\$this->".strtolower($this->objectName)."Id'\";";
		$this->string .= "\n\t\t\tforeach (\$Database->query(\$sql) as \$row)";
		$this->string .= "\n\t\t\t{";
		$this->string .= "\n\t\t\t\t\$count=\$row['COUNT'];";
		$this->string .= "\n\t\t\t}";
		...
		$this->string .= "\n\t\t\t{";
		$this->string .= "\n\t\t\t\t\$sql = (\"select max(".strtolower($this->objectName)."id) from ".strtolower($this->objectName)."\");";
		$this->string .= "\n\t\t\t\tforeach (\$Database->query(\$sql) as \$row)";
		$this->string .= "\n\t\t\t\t{";
