...
class Object
{
	...
	function CreatePreface()
	{
		...
		$this->string .= "\n* @link http://phpobjectgenerator.com/?language=php5&wrapper=pog&objectName=".urlencode($this->objectName)."&attributeList=".urlencode(var_export($this->attributeList, true))."&typeList=".urlencode(var_export($this->optionList, true));;
		$this->string .= "\n*/";
	}
	...
	function CreateGetAllFunction()
	{
		...
		$this->string .= "\n\t\t\$Database = new DatabaseConnection();";
		$this->string .= "\n\t\t\$query = \"select ".strtolower($this->objectName)."id from ".strtolower($this->objectName)." where `\".\$field.\"`\".\$comparator.\"'\".\$Database->Escape(\$fieldValue).\"' \$optionalConditions\";";
		$this->string .= "\n\t\t\$Database->Query(\$query);";
