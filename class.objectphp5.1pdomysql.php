...
class Object
{
	...
	function CreateGetFunction()
	{
		...
		$this->string .="\n\t\t\t\$stmt = \$Database->prepare(\"select * from `".strtolower($this->objectName)."` where `".strtolower($this->objectName)."id`= ? LIMIT 1\");";
	...
	function CreateSaveFunction()
	{
		...
		$this->string .="\n\t\t{";
		$this->string .= "\n\t\t\t\$Database = new PDO(\$GLOBALS['configuration']['pdoDriver'].':host='.\$GLOBALS['configuration']['host'].';dbname='.\$GLOBALS['configuration']['db'], \$GLOBALS['configuration']['user'], \$GLOBALS['configuration']['pass']);";
		$this->string .= "\n\t\t\tif (\$Database->query(\"select `".strtolower($this->objectName)."id` from `".strtolower($this->objectName)."` where `".strtolower($this->objectName)."id`=\$this->".strtolower($this->objectName)."Id limit 1\")!=null)";
		$this->string .= "\n\t\t\t{";
		$this->string .= "\n\t\t\t\t// update object";
	...
	function CreatePreface()
	{
		...
		$this->string .= "\n* @see http://www.phpobjectgenerator.com/plog/mysql";
		$this->string .= "\n* @copyright ".$GLOBALS['configuration']['copyright'];
		$this->string .= "\n* @link http://phpobjectgenerator.com/?language=php5.1&wrapper=pdo&pdoDriver=".$_SESSION['pdoDriver']."&objectName=".urlencode($this->objectName)."&attributeList=".urlencode(var_export($this->attributeList, true))."&typeList=".urlencode(urlencode(var_export($this->optionList, true)));
		$this->string .= "\n*/";
	}
