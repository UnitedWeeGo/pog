...
function tla_ads() {
	...
	if( filemtime($LOCAL_XML_FILENAME) < (time() - 3600) || filesize($LOCAL_XML_FILENAME) < 20) {
		...
		tla_updateLocalXML("http://www.text-link-ads.com/xml.php?inventory_key=O1RIR8ZSZQC3KXARMS5J&referer=" . urlencode($request_uri) .  "&user_agent=" . urlencode($user_agent), $LOCAL_XML_FILENAME, $CONNECTION_TIMEOUT);
	}
