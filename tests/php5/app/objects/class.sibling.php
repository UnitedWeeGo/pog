...
* @link http://www.phpobjectgenerator.com/?language=php5&wrapper=pog&objectName=sibling&attributeList=array+%28%0A++0+%3D%3E+%27object%27%2C%0A++1+%3D%3E+%27attribute%27%2C%0A%29&typeList=array+%28%0A++0+%3D%3E+%27JOIN%27%2C%0A++1+%3D%3E+%27VARCHAR%28255%29%27%2C%0A%29
*/
	...
	*/
	function Save($deep = true)
	{
		...
		if (Database::Rows($result) > 0)
		{
			...
			`attribute`='".POG_Base::Escape($this->attribute)."' where `siblingid`='".$this->siblingId."'";
		}
		else
		{
		...
		if ($this->siblingId == "")
		{
			$this->siblingId = Database::GetCurrentId($connection);
		}
		if ($deep)
		{
		...
		while($rows = mysql_fetch_assoc($result))
		{
			...
			foreach ($object->pog_attribute_type as $attribute_name => $attrubute_type)
			{
				if ($attrubute_type[1] != "HASMANY" && $attrubute_type[1] != "JOIN")
				{
					...
					$object->{$attribute_name} = POG_Base::Unescape($rows[strtolower($attribute_name)]);
				}
			}
			$objectList[] = $object;
		}
